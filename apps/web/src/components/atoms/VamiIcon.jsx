...
const iconPaths = {
  ...
  info: (
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M11.25 11.25l.041-.02a.75.75 0 112.593-.04a9 9 0 11-18.634 0zm0-3.75h.008v.008H12V7.5z"
    />
  ),
  edit: (
    <path
  ...
  minus: <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 12H4.5" />,
};

/**
 * VamiIcon component rendering standard outline SVG vector definitions.
 * Maintains a built-in sprite dictionary for portable, self-contained loading.
 *
 * @param {Object} props
 * @param {string} props.name - The icon key identifier
 * @param {string} [props.size='md'] - Sizing option ('sm', 'md', 'lg', 'xl')
 * @param {string} [props.className=''] - Additional CSS classes
 * @returns {React.JSX.Element}
 */
export function VamiIcon({ name, size = "md", className = "", ...props }) {
  const sizes = {
    sm: "h-4 w-4",
    md: "h-6 w-6",
    lg: "h-8 w-8",
  ...
  const selectedPath = iconPaths[name];

  if (!selectedPath) {
    console.warn(
      `VamiIcon: Icon name "${name}" is not defined in the sprite dictionary.`,
    );
    return null;
  }
  ...
  return (
    <svg
      ...
      strokeWidth="1.5"
      stroke="currentColor"
      className={`${sizeClass} inline-block select-none shrink-0 align-middle ${className}`}
      {...props}
    >
      {selectedPath}
    </svg>

      ...
      <main className="mx-auto max-w-3xl space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-border-default">
            <div>
              ...
              <p className="text-sm text-ink-600 mt-1">
                You are successfully authenticated using Magic Link passwordless
                security.
              </p>
            </div>

            {/* User Avatar Circle / Initials fallback */}
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-amber-500 font-ui text-xl font-extrabold text-ink-900 shadow-inner">
        ...
        <section className="rounded-xl border border-border-default bg-surface-elevated p-8 shadow-sm transition-colors duration-300 space-y-4">
          ...
          <p className="text-xs text-ink-600 leading-relaxed">
            Verify the secure local flow: the short-lived Bearer access token is
            held in-memory (Zustand state), and the long-lived refresh token
            session runs from an HTTP-only Lax cookie. Use the controls below to
            trigger custom interactions.
          </p>
